...
class LinearRegressionRegular:
    ...
    def fit(self, alpha=0):
        identidade = alpha*np.eye(self.X_train.shape[1])
        
        if self.fit_intercept:
            identidade[0, 0] = 0
        
        self.beta_hat = np.linalg.inv((self.X_train.T @ self.X_train) + identidade) @ self.X_train.T @ self.y_train
